...
pub struct PositionSnapshot {
    ...
    /// 合约乘数，**SCALE 标度定点值**，与 `TradingInstrumentSpec::contract_size`
    /// 同一约定（1.0 张 = `SCALE`）。历史上这里混用过普通整数，令名义额在
    /// 风控规则里被放大或缩小 1e9 倍；统一为定点值后不再需要调用方各自折算。
    pub multiplier: i128,
    pub long_qty: i128,
    pub short_qty: i128,
}
...
impl PositionSnapshot {
    ...
    /// `multiplier` 是 SCALE 标度的合约乘数，直接取
    /// `TradingInstrumentSpec::contract_size`。
...
impl RiskRule for MaxNotionalRule {
    ...
    fn check_with_price(
        ...
        // 名义额 = qty * price / SCALE * contract_size / SCALE，与
        // `TradingInstrumentSpec::notional` 使用同一标度约定。
        let leg_notional = |qty_abs: i128| {
            (qty_abs.saturating_mul(px.raw()) / SCALE).saturating_mul(multiplier) / SCALE
        };
        let current_notional = leg_notional(current_abs);
        let projected_notional = leg_notional(projected_abs);
...
mod tests {
    ...
    fn market_order_requires_reference_price_for_notional_risk() {
        ...
        assert!(gate
            .check_with_price(
                &market,
                &PositionSnapshot::new_with_multiplier(0, 0, 10 * SCALE),
