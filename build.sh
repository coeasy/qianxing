...
# 步骤编号与 build.bat 逐项对齐：[0/8] 选解释器，[1/8]..[8/8] 是八道发布门禁。
set -euo pipefail
...
if ! "$QX_PY" -c "import tzdata" >/dev/null 2>&1; then
  ...
  echo "       缺它时 [5/8] 的 A 股用例会以 ZoneInfoNotFoundError 失败" >&2
  echo "       修法: \"$QX_PY\" -m pip install tzdata" >&2
  exit 1
fi
echo "[0/8] Python 解释器 = $QX_PY (版本 $QX_PY_VER)"

# 与 build.bat 同一收口（V12 §19 #133）：[3/8] 与 [7/8] 由 Rust 侧自己起 Python，而 Rust 只读
